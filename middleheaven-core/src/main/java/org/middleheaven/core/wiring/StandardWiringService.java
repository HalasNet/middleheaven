		...
		for (WiringItemBundle bundle : this.bundles){
			...
			for (WiringItem item : bundle) {
				...
				if (item.isType()) {
					...
					// if is component
					if (Introspector.of(type).isAnnotationPresent(Component.class)){
	...
	public class StandardInstanceFactory implements InstanceFactory{
		...
		private Object obtainCyclicProxy(ReflectedClass<?> contract, Key key) {
			...
			if (contract.isInterface()){
				...
				Sequence<Object> params =c.getParameters().asEnumerable().map( p -> getInstance(WiringQuery.search(p.getType())))
