...
public class WiringTestCase extends MiddleHeavenTestCase {
	...
	public void testWiringServiceWithParams(){
		...
		assertNotNull(model.getProducingWiringPoint());
		assertTrue(model.getProducingWiringPoint().getParamsSpecifications().tryFirst().isPresent());
		assertFalse("construtor injection point as no params", model.getProducingWiringPoint().getParamsSpecifications().findFirst().getParams().isEmpty());
