...
public class PresenterWebCommandMapping implements WebCommandMapping {
	...
	protected final Outcome executeAction(HttpServerContext context){ 
		...
		try {
			...
			Sequence<Object> args = inicilizeActionParameters(actionMethod, context);
	...
	private Sequence<Object> inicilizeActionParameters(ReflectedMethod action,HttpServerContext context){
		Sequence<ReflectedParameter> parameters = action.getParameters();
		...
		return parameters.map(parameter -> {
			...
			if (type.isAssignableFrom(WebContext.class)){
				...
				if (context instanceof RequestResponseWebContext){
					return ((RequestResponseWebContext)context).getServletRequest();
				} else {
				...
				if (context instanceof RequestResponseWebContext){
					return ((RequestResponseWebContext)context).getServletResponse();
				} else {
					throw new IllegalStateException("Is not possible to inject " + HttpServletResponse.class.getName() + " on current environment");
				}
			} 
			
			final Sequence<Annotation> annotations = parameter.getAnnotations();
			if (annotations.isEmpty()){
				return beanLoader.loadBean(context, type.getReflectedType(), new Annotation[0]);
			}

			for (Annotation anot : annotations){
				if (anot instanceof PathVariable){
					String pathVariableName = ((PathVariable) anot).value();
					return readPathVariable(pathVariableName ,  type.getReflectedType(), variables);
				} else if (anot instanceof In){
					return context.getAttributes().getAttribute(((In) anot).value(), type.getReflectedType());
				}
			}
			throw new IllegalStateException("Is not possible to inject parameter number " + parameter.getIndex() + " on current environment");
