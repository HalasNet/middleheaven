...
//	}
	...
	public Complex plus(Number n) {
		return this.plus(new RealPairComplex(Real.valueOf(n), RealAlgebricStructure.getInstance().zero()));
	}

	@Override
	public Complex minus(Number n) {
		return this.minus(new RealPairComplex(Real.valueOf(n), RealAlgebricStructure.getInstance().zero()));
	}

	@Override
	public Complex times(Number n) {
		return this.times(new RealPairComplex(Real.valueOf(n), RealAlgebricStructure.getInstance().zero()));
	}

	@Override
	public Complex over(Number n) {
		return this.over(new RealPairComplex(Real.valueOf(n), RealAlgebricStructure.getInstance().zero()));
	}
	...
	/**
	 * 
	 * @return the real parte has a {@link Int}.
	 */
	@Override
	public Int toInt() {
		return toReal().toInt();
	...
	@Override
	public Complex abs() {
		return  new RealPairComplex(this.real.times(this.real).plus(this.imaginary.times(this.imaginary)) , this.real.getAlgebricStructure().zero());
	}
	...
	@Override
	public int signum() {
		return this.isZero() ? 0 : ( this.equals(abs()) ? 1 : -1);
	}
