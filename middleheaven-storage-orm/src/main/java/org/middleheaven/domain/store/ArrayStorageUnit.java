...
public class ArrayStorageUnit implements StorageUnit {
	 ...
	 */
	@Override
	public <E> Enumerable<E> filter(Enumerable<E> all, EntityCriteria<E> c) {
		...
		for (StoreAction action : actions){
			...
			if (type.isInstance(candidate)){
				...
				if (persisted == null){
					...
					if (EntityInstanceEntityCriteriaInterpreter.interpret(c).test(candidate)){
		...
		return Enumerables.asEnumerable(result.values());
	}
