...
class MachineIOSystemManagedFileRepository extends AbstractManagedRepository implements ManagedFileRepository{
	...
	ManagedFilePath pathForFile(File systemFile) {
		Sequence<String> parts;
		try {
			String separator = File.separator;
			if ( separator.equals("\\")) {
				parts = Splitter.on('/').split(systemFile.getCanonicalPath().replace('\\', '/'));
			} else {
				parts =  Splitter.on(File.separator).split(systemFile.getCanonicalPath());
			}
			return new ArrayManagedFilePath(this, parts.get(0), parts.subSequence(1));
		} catch (IOException e) {
	...
	public ManagedFilePath getPath(String first, String... more) {
		 if ( root == null) {
			 ...
			 if (this.isDriveRoot(new File(first + "/"))) {
				 return new ArrayManagedFilePath(this, first, Sequences.of(more));
			 }
			 
			 return new ArrayManagedFilePath(this, null, Sequences.of(first).concat(Sequences.of(more)));
