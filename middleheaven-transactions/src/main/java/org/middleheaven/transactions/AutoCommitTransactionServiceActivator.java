	...
	private class TransactionInterceptor implements WiringInterceptor {
		...
		InterceptorProxyHandler {
			...
			protected boolean willIntercept(Object proxy,
					Sequence<Object> args, MethodDelegator delegator) throws Throwable {
				return names.contains(delegator.getName());
			}

			@Override
			protected Object doInstead(Object proxy, Sequence<Object> args, MethodDelegator delegator) throws Throwable {
		...
		public Object proxyfy (final Object original, ReflectedClass<?> type){
			...
			if (all.isEmpty()){
				...
				final Set<String> names = all.map( m ->  m.getName()).into(Sink.collections()).set();
