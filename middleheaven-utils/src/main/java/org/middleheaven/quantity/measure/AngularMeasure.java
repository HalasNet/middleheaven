...
public class AngularMeasure extends DecimalMeasure<Angle>   {
	...
	public static AngularMeasure radians(double amount){
		return new AngularMeasure(
				Real.valueOf(amount),
				RealAlgebricStructure.getInstance().zero(),
				SI.RADIANS
		);
	}
	...
	public AngularMeasure   one() {
		return new AngularMeasure (RealAlgebricStructure.getInstance().one(), RealAlgebricStructure.getInstance().zero(), this.unit);
	}

	public AngularMeasure   zero() {
		return new AngularMeasure (RealAlgebricStructure.getInstance().zero(), RealAlgebricStructure.getInstance().zero(), this.unit);
	}
	...
	public AngularMeasure reduce(){
		...
		if (this.unit.equals(SI.RADIANS)){
			range = Interval.between(RealAlgebricStructure.getInstance().zero(), Real.valueOf(Math.PI*2));
		} else { 
			range = Interval.between(RealAlgebricStructure.getInstance().zero(), Real.valueOf(360));
		}
