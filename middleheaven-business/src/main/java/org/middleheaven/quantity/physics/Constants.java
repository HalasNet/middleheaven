...
public class Constants {
	 ...
	 */
	public static final Measure<Velocity> c = DecimalMeasure.exact(299792458, SI.METER.over(SI.SECOND)).cast(Velocity.class);
	
	/**
	 *  Speed of light in vacuum , squared
	 */
	public static final	Measure<Velocity> c_square = c.times(c);
	
	/**
	 *  Magnetic constant 
	 */
	public static final	Measure<?> miu_zero = DecimalMeasure.exact(4e-7 * Math.PI, SI.NEWTON.over(SI.AMPERE.raise(2)));
	
	/**
	 *  Newtonian constant of gravitation 
	 */
	public static final	Measure<?> G = DecimalMeasure.measure(6.67428e-11, 1.0e-4, SI.METER.raise(3).over(SI.SECOND.raise(2)).over(SI.KILOGRAM) );
	
	/**
	 *  Electric constant, also known as permitivity of vacuum. (1/(miu_zero*c^2)) 
	 */
	public static final	Measure<?> epslion_zero = miu_zero.times(c_square).inverse();
}
