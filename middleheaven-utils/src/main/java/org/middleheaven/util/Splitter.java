...
public class Splitter {
	...
	public interface KeyValueSplitter {
		...
		public Enumerable<Association.Entry<String,String>> split(CharSequence charSequence);
	...
	public static class PairFunction implements Function<Association.Entry<String, String>, String> {
		...
		@Override
		public Association.Entry<String, String> apply(String value) {
			final String[] p = value.split(keyValueSeparator);
			return new Association.Entry<String, String>() {
	...
	private static class KeyValueSplitterImpl implements KeyValueSplitter {
		...
		public Enumerable<Association.Entry<String,String>> split(CharSequence charSequence) {
			return spliter
					.split(charSequence)
					.map(new PairFunction(keyValueSeparator, transform)).asEnumerable();
	...
	public Sequence<String> split(CharSequence charSequence) {
		...
		if (source.indexOf(delimiter) < 0) {
			...
			res = Sequences.of(source);
		}

		if (delimiter.equals(".")) {
			res = Sequences.of(source.split("\\."));
		} else if (delimiter.equals(" ")) {
			res = Sequences.of(source.split("\\s"));
		} else {
			res = Sequences.of(source.split(delimiter));
		}
