...
public class ClassIntrospector<T> extends Introspector{
	...
	public T newInstance (Sequence<Object> args){
		return type.newInstance(args.toArray());
	}
	
	public T newInstance (Object arg){
		return type.newInstance(Sequences.of(arg));
	}
	...
	public T newProxyInstance(ProxyHandler handler){
		return Reflector.getReflector().proxyType(type.getReflectedType(), handler, Sequences.empty());
	} 
	...
	public void invokeMain(String ... params) {
		try {
		   type.getMethod("main", String[].class).invokeStatic( Sequences.of(params));
