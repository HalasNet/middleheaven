...
public abstract class ManagedFileDirectory extends Directory {
  ...
  private static Sequence<String> listAll(ManagedFile dir) throws IOException {
    if (!dir.exists()) {
    ...
    // Exclude subdirs 
    return dir.children().filter( file -> file.getType().isFile()).map( file -> file.getPath().getFileName()).distinct().into(Sink.collections()).sequence();
  }
  ...
  public String[] listAll() throws IOException {
    ensureOpen();
    Sequence<String> seq = listAll(directory);
    String[] array = new String[seq.size()];
    return seq.toArray(array);
  }
