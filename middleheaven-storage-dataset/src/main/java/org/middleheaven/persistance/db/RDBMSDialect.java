...
public abstract class RDBMSDialect {
	...
	public EditionDataBaseCommand createCreateIndexCommand(Enumerable<DBColumnModel> indexComponents, boolean indexIsUnique){
		...
		if (indexComponents.isEmpty()){
			throw new IllegalModelMappingException("Cannot create an index without components");
		}

		final String tableName = indexComponents.findFirst().getTableModel().getName();
		...
		int size = Maybe.of(indexComponents.getSizeInfo()).maybeCast(PreciseEnumerableSize.class).map( s -> s.getValue().asInteger()).or(30);
		int maxCharsForName = Math.max(4, 26 / size); 
