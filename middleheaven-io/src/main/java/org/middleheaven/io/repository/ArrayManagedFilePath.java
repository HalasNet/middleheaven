...
public class ArrayManagedFilePath implements ManagedFilePath , Watchable {
	...
	public ArrayManagedFilePath(ManagedFileRepository repository, String root){
		this(repository, root, Sequences.empty());
	}
	
	public ArrayManagedFilePath(ManagedFileRepository repository, String root ,  Sequence<String> more){
		...
		if (names.isEmpty()){
			...
			String lastName = this.names.get(names.size()-1);
	...
	public ArrayManagedFilePath(ManagedFilePath path, String lastName) {
		...
		ListSequence<String> seq = new ListSequence<>(nameCount + 1);
		this.names  = seq;

		for (int i = 0 ; i < nameCount; i++){
			seq.set(i, path.getName(i));
		}

		seq.set(seq.size() -1 , lastName);
	...
	public ManagedFilePath getParent() {
		return new ArrayManagedFilePath(
				this.repository,
				this.root,
				this.names.subSequence(0, this.names.size() - 1)
		);
	}
	...
	public ManagedFilePath resolve(String name) {
		return new ArrayManagedFilePath(this.repository, this.root, this.names.concat(Splitter.on("/").split(name)));
	}
	...
	public ManagedFilePath resolve(ManagedFilePath path) {
		...
		ListSequence<String> newPath = new ListSequence<>(this.names.size() + nameCount);
		
		if (!names.isEmpty()){
			newPath.addAll(this.names);
		}

		for (int i = 0; i < nameCount; i++){
			newPath.add(path.getName(i));
		}
	...
	public ManagedFilePath resolveSibling(String name) {
		Sequence<String> pathNames = Splitter.on("/").split(name);
		
		Sequence<String> all = this.names.subSequence(0, this.names.size() - 1) // do not copy the last
		.concat(pathNames);
	...
	public String getFileName() {
		return names.isEmpty() ? null : this.names.asEnumerable().findLast();
	}
