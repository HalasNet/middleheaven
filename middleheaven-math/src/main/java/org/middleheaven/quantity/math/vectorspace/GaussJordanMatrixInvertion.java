...
public class GaussJordanMatrixInvertion implements MatrixInvertionAlgorithm{
	 ...
	 static <T extends FieldElement<T, ?>> EditableMatrix<T> augmentWithEntity(Matrix<T> m){
		
		final T ONE = m.getField().one();
		
		EditableMatrix<T> e = m.getAlgebricStructure().getVectorSpace().subMatrix(m.rowsCount(), m.columnsCount()*2);
	...
	public <F extends FieldElement<F,?>> Matrix<F> invert( Matrix<F> matrix){
		...
		EditableMatrix<F> A = augmentWithEntity(matrix);
		...
		return new DenseMatrix<F>(A,0 , A.columnsCount()/2, A.rowsCount(),A.columnsCount());
	}
	...
	// post: A[i][j]==1;
	private static <F extends FieldElement<F, ?>>  void divide(EditableMatrix<F> A, int i, int j,F ONE){
	...
	private static <F extends FieldElement<F, ?>>  void eliminate(EditableMatrix<F> A, int i, int j, F ZERO){
