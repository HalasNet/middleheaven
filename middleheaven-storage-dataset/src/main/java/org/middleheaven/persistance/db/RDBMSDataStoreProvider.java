			...
			return new DataStoreSchema(){
				...
				public void updateModel() throws ModelNotEditableException {
					...
					for (DBTableModel tm  : Function.getTableModels()){
						if (tm.isEmpty()){
							throw new IllegalModelMappingException("Table " + tm.getName() + " has no columns");
						}
						...
						if (existingModel != null) {
							...
							try {
								if (dbObject.getType().equals(DataBaseObjectType.TABLE)) {
									...
									for (DataBaseCommand idxCommand : tm.columns().groupBy(obj -> obj.getUniqueGroupName()).map( grouping -> dialect.createCreateIndexCommand(grouping.elements() , true)) ){
