...
public final class JavaPropertyAccessor implements ReflectedProperty {
	...
	public static JavaPropertyAccessor forTypeAndName(ReflectedClass<?> type, String propertyName){
		try {
			...
			for (ReflectedMethod method : type.getMethods()){
				...
				final Sequence<ReflectedParameter> parameters = method.getParameters();
				...
				} else if (parameters.isEmpty() && (method.getName().equalsIgnoreCase("get" + propertyName) ||  
	...
	public Object getValue(Object target) throws ReflectionException{
		...
		try {
			if (accessor!=null){
				return accessor.invoke(target, Sequences.empty());
			} else if (field !=null){
		...
		try {
			...
			if( value == null && getValueType().isPrimitive()){
				...
				if (modifier != null){
					...
					modifier.invoke(target, Sequences.of(value));
	...
	public Sequence<Annotation> getAnnotations() {
		Sequence<Annotation> all = Sequences.empty();
