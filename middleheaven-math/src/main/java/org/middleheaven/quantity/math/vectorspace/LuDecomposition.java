...
public final class LuDecomposition<F extends FieldElement<F, ?>> {
	...
	public static <R extends FieldElement<R, ?>> LuDecomposition<R> decompose(Matrix<R> LU){
		...
		EditableMatrix<R> L = LU.getAlgebricStructure().getVectorSpace().subMatrix(LU.columnsCount(),LU.columnsCount());
		EditableMatrix<R> U = LU.getAlgebricStructure().getVectorSpace().subMatrix(LU.columnsCount(),LU.columnsCount());
