...
public final class VersionReader {
    ...
    public static Version fromString(String versionStr){
        Sequence<String> parts = Splitter.on('.').trim().split(versionStr.trim());
        Version version = new Version(false);
        try{
            if (parts.size()>0){

                version.setMajor(Integer.parseInt(parts.get(0)));

            }
            if (parts.size() >1){

                version.setMinor(Integer.parseInt(parts.get(1)));

            }
            if (parts.size() > 2){

                version.setRevision(Integer.parseInt(parts.get(2)));

            }
            if (parts.size() >3){

                version.setBuild(Integer.parseInt(parts.get(3)));
            }
